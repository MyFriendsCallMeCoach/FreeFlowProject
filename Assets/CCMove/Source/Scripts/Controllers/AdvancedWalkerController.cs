...
namespace CMF
{
	...
	public class AdvancedWalkerController : Controller {
		...
		public static AdvancedWalkerController instance;
		...
		protected CapsuleCollider CC_Capsule;
		public Rigidbody CC_Rigid;
		...
		[Header("Schmovement Ablities")]

		public LayerMask GrabbableLedge;
		...
		[HideInInspector]
		public int JumpsLeft; //We keep track of how many jumps are left

		public int NumberOfJumps = 2; //We set the amount of times the player character can jump
		...
		public float gravity = 30f, GravityGlide = 10f, GravityDefault;
		...
		void Awake () {
			...
			CC_Rigid = GetComponent<Rigidbody>();
        ...
        void HandleJumping()
        {
			//we reset the number of jumps left when the character controller is grounded
			if (currentControllerState == ControllerState.Grounded || currentControllerState == ControllerState.LedgeIdle)
				JumpsLeft = NumberOfJumps;
			CMF.CharacterDefaultInput.instance.Jump_Input = false;
			//gravity = 30f;



			if (currentControllerState == ControllerState.Grounded || (currentControllerState != ControllerState.Jumping && JumpsLeft > 0 || currentControllerState == ControllerState.LedgeIdle))
            {
			...
			else if(currentControllerState == ControllerState.Falling && JumpsLeft <= 0 || currentControllerState == ControllerState.Rising && JumpsLeft <= 0)
            {
				if ((jumpKeyIsPressed == true || jumpKeyWasPressed) && !jumpInputIsLocked)
				{
					//MeshTrail.instance.PlayerMeshTrail();
					gravity = GravityGlide;
					movementSpeed = DefaultMovementSpeed;
        ...
        void HandleMomentum()
		{
			...
			if(currentControllerState == ControllerState.Grounded && VectorMath.GetDotProduct(_verticalMomentum, tr.up) < 0f || currentControllerState == ControllerState.LedgeIdle)
			...
			//if controller is on a ledge we remove all vertical velocity
			if (currentControllerState == ControllerState.LedgeIdle)
			{
				momentum = Vector3.zero;//new Vector3(0,0,0);
		...
		void OnJumpStart()
		{
			CharAnimator.SetBool("LedgeGrabbed", false);
		...
		//we handle the jump input, set up so player cannot dodge while jumping
        ...
        {
			return (currentControllerState == ControllerState.LedgeIdle);

		}

		public void GrabLedge(Vector3 handpos, Quaternion handrot, PlayerLedgeChecker currentLedge)
		{
			IsGrabbingLedge();
